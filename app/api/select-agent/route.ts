...
export async function POST(req: NextRequest) {
	try {
		...
		// Guardrail: Reject low-confidence queries
		if (!confidence || confidence < 5) {
			return NextResponse.json(
				{
					error: 'irrelevant_query',
					message: `I can only help with:
- Generating LinkedIn posts based on your past content and style
- Polishing existing LinkedIn posts to make them more engaging

Please keep your queries focused on these use cases.`,
				},
				{ status: 400 }
			);
		}
